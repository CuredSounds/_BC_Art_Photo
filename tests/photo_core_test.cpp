...
Image makeRgbImage(int width, int height, const std::vector<std::uint8_t>& pixels) {
    assert(static_cast<int>(pixels.size()) == width * height * 3);
    return Image{width, height, 3, pixels};
}

void testBrightness() {
    auto image = makeRgbImage(1, 1, {10, 20, 30});
    bc_art_photo::applyBrightness(image, 20);
    assert((image.pixels == std::vector<std::uint8_t>{30, 40, 50}));
}

void testContrast() {
    auto image = makeRgbImage(1, 1, {100, 128, 200});
...
void testGrayscale() {
    auto image = makeRgbImage(1, 1, {255, 0, 0});
